...
class ORMFixture:
    ...
    def convert_groups_to_model(self, groups):
        def convert(group):
            return Group(gr_id=str(group.id), gr_name=group.name, gr_header=group.header, gr_footer=group.footer)
    ...
    def get_contacts_in_group(self, group):
        orm_group = list(select(g for g in ORMFixture.ORMGroup if g.id == group.gr_id))[0]
